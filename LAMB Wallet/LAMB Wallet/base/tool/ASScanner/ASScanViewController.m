...
@interface ASScanViewController ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, AVCaptureMetadataOutputObjectsDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) ASScanView *scanRectView;
...
- (instancetype)initWithQrType:(ASScanType)type onFinish:(void (^)(NSString *result, NSError *error))finish {
    self = [super init];
    if (self) {
        self.scanType = type;
...
- (void)viewDidLoad {
    ...
    self.title = ASLocalizedString(@"二维码");
...
- (void)initScanType{
    if (self.scanType == ASScanTypeAll) {
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        [self drawBottomItems];
    } else if (self.scanType == ASScanTypeQrCode) {
        self.output.metadataObjectTypes = @[AVMetadataObjectTypeQRCode];
        self.title = ASLocalizedString(@"二维码");
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        _tipTitle.text = ASLocalizedString(@"将取景框对准二维码,即可自动扫描");
    ...
    } else if (self.scanType == ASScanTypeBarCode) {
        ...
        self.output.rectOfInterest = _scanRect;
        [self.scanRectView setScanType: ASScanTypeBarCode];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    {
        [self showError:ASLocalizedString(@"图片中未识别到二维码")];
        return;
    }
...
//绘制扫描区域
- (void)drawScanView {
    _scanRectView = [[ASScanView alloc] initWithFrame:self.view.frame style:@""];
...
- (void)drawBottomItems
{
    ...
    [_scanTypeQrBtn setTitle:ASLocalizedString(@"二维码") forState:UIControlStateNormal];
...
- (void)setNavItem:(ASScanType)type {
    if(type == ASScanTypeBarCode) {
        [self.navigationItem setRightBarButtonItem:nil];
    } else {
        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:ASLocalizedString(@"相册") style:UIBarButtonItemStylePlain target:self action:@selector(openPhoto)];
...
- (void)qrBtnClicked:(UIButton *)sender {
    ...
    if (self.delayQRAction) return;
    
    [sender setSelected:YES];
    [_scanTypeBarBtn setSelected:NO];
    [self changeScanCodeType:ASScanTypeQrCode];
    [self setNavItem:ASScanTypeQrCode];
    self.delayQRAction = YES;
...
- (void)barBtnClicked:(UIButton *)sender {
    if (sender.selected) return;
    if (self.delayBarAction) return;
    
    [sender setSelected:YES];
    [_scanTypeQrBtn setSelected:NO];
    [self.scanRectView stopAnimating];
    [self changeScanCodeType:ASScanTypeBarCode];
    [self setNavItem:ASScanTypeBarCode];
    self.delayBarAction = YES;
    __weak __typeof(self)weakSelf = self;
    [self performTaskWithTimeInterval:3.0f action:^{
...
- (void)changeScanCodeType:(ASScanType)type {
    [self.session stopRunning];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        weakSelf.tipTitle.text = type == ASScanTypeQrCode ? ASLocalizedString(@"将取景框对准二维码,即可自动扫描") : ASLocalizedString(@"将取景框对准条码,即可自动扫描");
        [weakSelf.session startRunning];
    });
    
    [UIView animateWithDuration:0.3 animations:^{
        weakSelf.tipTitle.center = CGPointMake(weakSelf.view.center.x, weakSelf.view.center.y + scanSize.height/2 + 25);
    }];
}
...
- (void)openPhoto
{
    ...
    {
        NSString *tipString = [NSString stringWithFormat:@"%@",ASLocalizedString(@"请到手机系统的\n【设置】->【隐私】->【相册】\n对XX开启相机的访问权限")];
        NSString *tipMessage = [tipString stringByReplacingOccurrencesOfString:@"XX" withString:appName];
        [self showError:tipMessage andTitle:ASLocalizedString(@"相册读取权限未开启")];
    }
}
...
- (BOOL)isAvailableCamera {
    if([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        ...
        if (authorizationStatus == AVAuthorizationStatusRestricted ||
            ...
            NSString *tipString = [NSString stringWithFormat:@"%@",ASLocalizedString(@"请到手机系统的\n【设置】->【隐私】->【相册】\n对XX开启相机的访问权限")];
            NSString *tipMessage = [tipString stringByReplacingOccurrencesOfString:@"XX" withString:appName];
            [self showError:tipMessage andTitle:ASLocalizedString(@"相机权限未开启")];
            return NO;
        }else{
            return  YES;
        }
...
- (void)showError:(NSString*)str andTitle:(NSString *)title
{
    ...
    UIAlertAction *action1 = ({
        UIAlertAction *action = [UIAlertAction actionWithTitle:ASLocalizedString(@"知道了") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
+ (void)recognizeQrCodeImage:(UIImage *)image onFinish:(void (^)(NSString *result))finish {
    [[[ASScanViewController alloc] init] recognizeQrCodeImage:image onFinish:finish];
}
