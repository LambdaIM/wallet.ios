...
#warning 此处居中需求
        BOOL isNeedCenter = YES;
        
        for (ZJTitleView *titleView in self.titleViews) {
            titleW = [self.titleWidths[index] floatValue];
            if (isNeedCenter && index == 0) {
                NSNumber *sum = [self.titleWidths valueForKeyPath:@"@sum.self"];
                titleX =
                (kScreenW - sum.floatValue - (self.titleViews.count-1)*self.segmentStyle.titleMargin)*0.5;
                lastLableMaxX = titleX + (titleW + addedMargin + self.segmentStyle.titleMargin);
