...
-(void)buildUI{
    ...
    flowLayout.itemSize = CGSizeMake(cellWidth,cellWidth/3.0f*1.15);
    ...
    if (!self.enableSelect) {
        flowLayout.headerReferenceSize = CGSizeMake(self.bounds.size.width, 0);
    }else{
    ...
    self.collectionView.backgroundColor = [UIColor colorWithHexString:@"C5CAD4"];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        headerView.title = @"助记词校验";
        headerView.subTitle = @"";
    }else{
        headerView.title = @"助记词";
        headerView.subTitle = @"";
    }
    return headerView;
}
