...
@interface KBWithdrawRewardVC ()<ZJScrollPageViewDelegate>
@property(nonatomic, weak) ZJScrollPageView *sv;

@end

@implementation KBWithdrawRewardVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self refreshUIRealTime];
    self.title = ASLocalizedString(@"提取奖励");
    ...
    [[ZJScrollPageView alloc] initWithFrame:scrollPageViewF
                               segmentStyle:[self style]
                                     titles:@[ASLocalizedString(@"奖励"), ASLocalizedString(@"节点收益")]
...
- (UIViewController<ZJScrollPageViewChildVcDelegate> *)childViewController:(__kindof UIViewController<ZJScrollPageViewChildVcDelegate> *)reuseViewController forIndex:(NSInteger)index {
    ...
    KBWithdrawRewardSubVC<ZJScrollPageViewChildVcDelegate> *vc = reuseViewController;
    
    if (!vc) {
        vc = [KBWithdrawRewardSubVC new];
    }
    vc.m_nodeRevenue = (index == 1);
    return vc;
}
